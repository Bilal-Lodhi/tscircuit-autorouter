...
import {
  ...
  convertSolvedRoutesToSerializedSolvedRoutes,
} from "lib/solvers/PortPointPathingSolver/hgportpointpathingsolver/HyperGraphSectionOptimizer2_PortPointPathing"
import {
  buildHyperGraph,
...
type PipelineStep<T extends new (...args: any[]) => BaseSolver> = {
  ...
  getConstructorParams: (
    instance: AutoroutingPipelineSolver4,
  ) => ConstructorParameters<T>
  onSolved?: (instance: AutoroutingPipelineSolver4) => void
}
...
>(
  solverName: keyof AutoroutingPipelineSolver4,
  solverClass: T,
  getConstructorParams: (instance: AutoroutingPipelineSolver4) => P,
  opts: {
    onSolved?: (instance: AutoroutingPipelineSolver4) => void
  } = {},
): PipelineStep<T> {
...
const getHyperGraphPortPointPathingFlags = () => ({
  FORCE_CENTER_FIRST: true,
  RIPPING_ENABLED: true,
})

const getHyperGraphPortPointPathingWeights = () => ({
  SHUFFLE_SEED: 0,
  MEMORY_PF_FACTOR: 4,
  ...
  START_RIPPING_PF_THRESHOLD: 0.01,
...
const createHyperGraphPortPointPathingSolverInput = (
  cms: AutoroutingPipelineSolver4,
  input: {
    graph: HyperGraphHg
  ...
  inputSolvedRoutes: input.inputSolvedRoutes,
  colorMap: cms.colorMap,
  layerCount: cms.srj.layerCount,
  effort: cms.effort,
  flags: getHyperGraphPortPointPathingFlags(),
  weights: getHyperGraphPortPointPathingWeights(),
})

export class AutoroutingPipelineSolver4 extends BaseSolver {
  ...
  hyperGraphSectionOptimizer?: HyperGraphSectionOptimizer2_PortPointPathing
  viaDiameter: number
  ...
  pipelineDef = [
    ...
    definePipelineStep(
      ...
      (cms) => {
        ...
        return [
          createHyperGraphPortPointPathingSolverInput(cms, {
            graph,
            connections,
          }),
        ]
        ...
        return [
          {
            inputGraph: convertHyperGraphToSerializedHyperGraph(
              portPointSolver.graph,
            ),
            inputConnections: convertConnectionsToSerializedConnections(
              portPointSolver.connections,
            ),
            inputSolvedRoutes: convertSolvedRoutesToSerializedSolvedRoutes(
              portPointSolver.solvedRoutes,
            ),
            sectionExpansionHops: 1,
            maxTargetRegionAttempts: Math.max(50, 50 * cms.effort),
            maxSectionAttempts: Math.max(50, 50 * cms.effort),
            minCentralRegionCost: 0.05,
            colorMap: cms.colorMap,
            effort: cms.effort,
            layerCount: cms.srj.layerCount,
            flags: getHyperGraphPortPointPathingFlags(),
