...
import { createTrivialRoutesForSingleConnectionNode } from "./createTrivialRoutesForSingleConnectionNode"
...
export class HighDensitySolver extends BaseSolver {
  ...
  _step() {
    ...
    // For a node with 0/1 unique connection names, routing is trivial:
    // connect the points with straight-line intra-node routes and any
    // required single via layer transitions.
    if (uniqueConnectionNames.size <= 1) {
      this.routes.push(
        ...createTrivialRoutesForSingleConnectionNode({
          node,
