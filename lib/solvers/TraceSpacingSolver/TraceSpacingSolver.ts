    ...
    for (
      ...
      for (let pointIndex = 0; pointIndex < points.length; pointIndex++) {
        ...
        for (const obstacle of this.obstacles) {
          ...
          if (isObstacleConnectedToRoute(obstacle, rootName, this.connMap)) {
            continue
          }
...
const isObstacleConnectedToRoute = (
  ...
  for (const connectedId of obstacle.connectedTo) {
    if (connMap.areIdsConnected(rootConnectionName, connectedId)) {
      return true
    }
  }
  for (const connectedId of obstacle.offBoardConnectsTo ?? []) {
    if (connMap.areIdsConnected(rootConnectionName, connectedId)) {
      return true
    }
  }
